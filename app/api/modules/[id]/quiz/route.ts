...
// GET /api/modules/[id]/quiz — Fetch quiz questions for a module
export async function GET(
  ...
  try {
    ...
    const kuis = content?.modul?.kuis_akhir || {
      judul: `Evaluasi Akhir - ${mod.title}`,
      nilai_lulus: 70,
      waktu_menit: 120,
      soal: [],
    };
    ...
    return NextResponse.json({ error: 'Gagal mengambil data kuis: ' + err.message }, { status: 500 });
  }
}

// PUT /api/modules/[id]/quiz — Save/Import quiz questions for a module directly to DB & Disk
export async function PUT(
  ...
  try {
    ...
    if (!contentJson.modul.kuis_akhir) {
      contentJson.modul.kuis_akhir = {
        judul: `Evaluasi Akhir - ${mod.title}`,
        nilai_lulus: 70,
        waktu_menit: 120,
        soal: [],
    ...
    if (mode === 'append') {
      finalSoal = [...(contentJson.modul.kuis_akhir.soal || []), ...soal];
    } else {
      finalSoal = soal;
    }

    contentJson.modul.kuis_akhir.soal = finalSoal;
    ...
    return NextResponse.json({
      ok: true,
      message: `Berhasil menyimpan ${finalSoal.length} soal kuis ke database!`,
      totalQuestions: finalSoal.length,
      moduleTitle: mod.title,
    });
  } catch (err: any) {
    console.error('Error updating quiz endpoint:', err);
    return NextResponse.json({ error: 'Gagal menyimpan data kuis: ' + err.message }, { status: 500 });
  }
}
